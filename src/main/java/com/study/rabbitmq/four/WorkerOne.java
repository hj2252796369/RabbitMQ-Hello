...
public class WorkerOne {
    ...
    public static void main(String[] args) throws IOException, TimeoutException {
        ...
        System.out.println("C1接收信息，接收信息时间短");

        channel.basicQos(7);
        channel.basicConsume(QUEUE_NAME, false, (consumerTag, message)->{
            try {
                System.out.println("C1接收到的信息");
                TimeUnit.SECONDS.sleep(1);
                System.out.println("C1接收信息 ===> " + new String(message.getBody(), "UTF-8"));
                channel.basicAck(message.getEnvelope().getDeliveryTag(), false);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }, (consumerTag)->{
            System.out.println("C1取消接收信息 ===> " + consumerTag.toString());
        });
